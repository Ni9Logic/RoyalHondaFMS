...
const createTransaction = async (amount: number, email: string, transactionType = "Deposit") => {
    try {
...
const Deposits: React.FC<DepositProps> = ({currentUser}) => {
    ...
    const onSubmit: SubmitHandler<FormValues> = async (data: FormValues) => {
        try {
            // When the user clicks on submit button in this case deposit button we'll set the loading of button to true
            setLoading(true);
            ...
            // Checking that the amount shall only be in positive integer
            if (data.amount <= 0) {
                toast.error('Invalid Amount');
                setLoading(false);
                return null;
            ...
            // Now time to deposit
            const update = await deposit(data.amount, currentUser?.email);

            if (update) {
                // Now since user is updated it is time we also create a transaction
                //@ts-ignore
                const transaction = await createTransaction(data.amount as number, currentUser?.email as string);
