...
export default function PAGE() {
    ...
    const getEstimateByIdToEdit = async () => {
        // @ts-ignore
        let id = parseInt(searchPararms.get('id')?.toString()) - 1;
        ...
        axios.post('/api/getEstimate', { id: parseInt(searchPararms.get('id')?.toString()) })
            .then((response: AxiosResponse) => {
                ...
                setValue('isRoyal', myData?.isRoyal);
                setValue('cKiloMeters', myData?.cKiloMeters);
                setValue('CreatedAt', myData?.CreatedAt);
                setValue('DiscountServices', myData?.DiscountServices);
                setValue('DiscountEstimate', myData?.DiscountEstimate);
                setValue('DiscountServicesFigure', myData?.DiscountServicesFigure);
                setValue('DiscountEstimateFigure', myData?.DiscountEstimateFigure);
                setValue('TotalServiceFee', myData?.TotalServiceFee);
                ...
                setcKiloMeters(myData?.cKiloMeters);
                setDiscountServices(myData?.DiscountServices);
                setDiscountEstimate(myData?.DiscountEstimate);
                setDiscountServicesFigure(myData?.DiscountServicesFigure);
    ...
    useEffect(() => {
        // @ts-ignore
        setEstId(parseInt(searchPararms.get('id')?.toString()) - 1)
        const fetchData = async () => {
            await getAllSurveyors();
            await getAllInsurancess();
            await getEstimateByIdToEdit();
        }
                        ...
                        <div className={"mt-32 h-full w-full flex flex-col gap-4"}>
                            <div>
                                ...
                                <Input onChange={(e) => {
                                    ...
                                    className={"mt-2 flex w-3/12 ml-auto"}
                                    type="number"
                                    placeholder={"Discount Parts (Figure)"}
                                    defaultValue={DiscountEstimateFigure} />
                            </div>
                            ...
                            <div>
                                ...
                                <div className={"gap-2 flex flex-row"}>
                                    ...
                                    <Button type={"button"} onClick={() => {
                                        ...
                                        // @ts-ignore
                                        setValue('id', parseInt(searchPararms.get('id')?.toString()));
